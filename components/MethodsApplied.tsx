  ...
  return (
    <section className="max-w-7xl mx-auto w-full px-6 md:px-12">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
        ...
        >
          <h2 
            className="text-3xl md:text-5xl font-medium text-white mb-8 leading-tight tracking-tight"
        ...
        <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-16">
          {methods.map((method, index) => (
            ...
            >
              {/* Animated Top Border */}
              <div className="h-[1px] w-full bg-white/20 mb-6 relative overflow-hidden">
                 <div className="absolute top-0 left-0 h-full w-full bg-[#d4af37] -translate-x-full group-hover:translate-x-0 transition-transform duration-700 ease-out" />
              </div>
              
              <h3 className="text-xl font-medium text-white mb-3 group-hover:text-[#d4af37] transition-colors duration-500">
                {method.title}
              </h3>
              
              {method.desc && (
                <p className="text-[#808080] leading-relaxed font-light">
                  {method.desc}
                </p>
              )}
            </motion.div>
