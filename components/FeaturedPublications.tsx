...
export default function FeaturedPublications({
  ...
  return (
    <section
      className={`w-full py-24 md:py-32 border-t ${isLight ? "border-black/10" : "border-white/10"}`}
    >
            ...
            >
              ...
              <div className="lg:col-span-8 flex flex-col gap-12">
                {group.items.map((item, pIndex) => (
                  ...
                  >
                    <h4
                      className={`text-[20px] md:text-[24px] leading-tight font-medium ${isLight ? "text-black" : "text-gray-200"} group-hover:${isLight ? "text-[#96750b]" : "text-white"} transition-colors duration-300 mb-4`}
                    >
                      {item.title}
                    </h4>

                    <div
                      className={`${isLight ? "text-[#666666]" : "text-[#a0a0a0]"} text-[15px] mb-6 font-light`}
                    >
                      {item.source}
                    </div>

                    <p
                      className={`${isLight ? "text-[#444444]" : "text-[#a0a0a0]"} leading-relaxed font-light mb-8 text-base`}
                    >
                      {item.description}
                    </p>
                    ...
                    {item.linkText && (
                      <Link
                        target="_blank"
                        href={item.href || "#"}
                        className={`inline-flex items-center gap-2 ${isLight ? "text-[#96750b] hover:text-[#7a5f09]" : "text-[#d4af37] hover:text-white"} transition-colors duration-300 text-[15px] group/link`}
                      >
                        {item.linkText}
                        <svg
