...
export default function OngoingProjects({ theme = "dark" }: { theme?: "light" | "dark" }) {
  ...
  return (
    <section className="w-full ">
      ...
      >
        <h2 
          className={`text-3xl md:text-5xl font-medium ${isLight ? "text-black" : "text-white"} leading-tight tracking-tight uppercase`}
      ...
      <div className={`w-full border-b ${isLight ? "border-black/10" : "border-white/20"}`}>
        <div className={`border-t ${isLight ? "border-black/10" : "border-white/20"} w-full`}>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-10% 0px" }}
          ...
          >
            ...
            {/* Content Column */}
            <div className={`w-full md:w-[60%] text-base leading-relaxed ${isLight ? "text-[#444444]" : "text-[#d1d1d1]"} font-light flex flex-col gap-6 items-start`}>
              <p>
              ...
              <div className="mt-4">
                <ActionText text="Read more" href="/protests-and-polls" theme={theme} />
              </div>
            </div>
          </motion.div>
        </div>
